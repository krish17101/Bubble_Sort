...
import { Step, StepHistory } from "./StepHistory.mjs";
...
function setDescription({ type, i, step }) {
    switch (type) {
        case "newPass":
...
const onNext = () => {
    ...
    if (!step)
        return;
    (_a = state.history) === null || _a === void 0 ? void 0 : _a.push({
        i: step.value.i,
        j: step.value.j,
        // @ts-ignore
    ...
    showStep.textContent = `${++state.step} / ${state.totalSteps}`;
    if (step === null || step === void 0 ? void 0 : step.done) {
        next.removeEventListener("click", onNext);
        next.addEventListener("click", onRestart);
        next.textContent = "🔁 Restart";
    }
...
const showStep = document.getElementById("atStep");
